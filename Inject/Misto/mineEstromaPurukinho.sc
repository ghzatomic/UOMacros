...
recallLine= '.recall 1,2';
//recallLineMine= '.recall 1,6'; //Lost Ore Mine
recallLineMine= '.recall 1,1';  //Monte BlackThorn
recallReg='.recall 1,2';
PickType = $0E85 ;
PickType2 = $0E86;
sunduk = $40141AD1 ;//?????? ? ??????? ?????????? ?????? ? ????? ?????.
bagpickaxe = $40141AD1 ;//????? ? ??????? ?? ??????? ????? ????? ??? ???????
bagjuvelir=$40141AD1 ;//????? ? ??????? ? ??????? ?????????? ????? ???????
IDBoxIngot=$40141AD1 ;  //ID ????? ??? ????? ?????? ? ?????? ?????
TinkerTool=$1EBC ;
ingot=$1BEF;
tong=$0FBB ;
savetime = 5000;
...
//   AddToSystemJournal('175 ??? ?????');
     ...
     begin 
     while ( mana < 100) and not dead  do
     begin
     UseSkill('meditation');
     wait(2000);
     end;
...
if CHARNAME = 'Estromatolilito' then
begin
BugOre:=$400BEA26  ;
end;
